 ...
 * Deliberately simple vector shapes: cheap to draw and on-brand rather than a heavy raster.
 */
@Composable
fun IntroIllustration(art: IntroArt, modifier: Modifier = Modifier) {
    ...
    Canvas(
        ...
        val soft = colors.pink.copy(alpha = 0.12f)
        when (art) {
            IntroArt.RHYTHM -> drawRhythm(colors.pink, colors.accent, soft)
            IntroArt.TRACK -> drawTrack(colors.pink, colors.accent, soft)
            IntroArt.INSIGHT -> drawInsight(colors.pink, colors.accent, soft)
        }
    }
}

/** Concentric "cycle" rings with a body-rhythm wave and a moon/sun pair. */
private fun DrawScope.drawRhythm(pink: Color, accent: Color, soft: Color) {
    val c = center
    val r = size.minDimension * 0.36f
    drawCircle(soft, radius = r * 1.25f, center = c)
    drawCircle(pink, radius = r, center = c, style = Stroke(width = r * 0.06f))
    drawCircle(accent, radius = r * 0.62f, center = c, style = Stroke(width = r * 0.05f))
    ...
    drawPath(wave, pink, style = Stroke(width = r * 0.05f))
    drawCircle(pink, radius = r * 0.1f, center = Offset(c.x - r, c.y - r))
    drawCircle(accent, radius = r * 0.1f, center = Offset(c.x + r, c.y - r * 0.8f))
}

/** A small calendar grid with one highlighted day and a mood dot row beneath. */
private fun DrawScope.drawTrack(pink: Color, accent: Color, soft: Color) {
    ...
    for (row in 0 until rows) {
        for (col in 0 until cols) {
            ...
            drawCircle(
                if (highlighted) pink else pink.copy(alpha = 0.28f),
                radius = cell * if (highlighted) 0.34f else 0.22f,
                center = Offset(cx, cy),
            )
        }
    }
    val moodY = top + rows * cell + cell
    val moods = listOf(pink, accent, pink.copy(alpha = 0.5f))
    moods.forEachIndexed { i, color ->
        drawCircle(color, radius = cell * 0.3f, center = Offset(left + cell + i * cell * 1.5f, moodY))
    }
}

/** A rising insight line over a soft panel with an accent "spark" node. */
private fun DrawScope.drawInsight(pink: Color, accent: Color, soft: Color) {
    ...
    drawRoundRect(
        ...
        cornerRadius = androidx.compose.ui.geometry.CornerRadius(panelH * 0.16f),
    )
    ...
    drawPath(path, pink, style = Stroke(width = panelH * 0.05f))
    points.forEach { (fx, fy) ->
        drawCircle(pink, radius = panelH * 0.04f, center = Offset(panelLeft + fx * panelW, panelTop + fy * panelH))
    }
