...
import { Icon, RulerPicker } from '@/shared/ui';
...
function PersonalForm({ profile }: { profile: UserProfile }) {
  ...
  const handleSave = () => {
    ...
    update.mutate(payload, { onSuccess: () => closeSheet() });
  };
  ...
  return (
    <>
      <div className="prof-edit-scroll">
        {/* Name */}
...
/** Editor for the account's personal info: name, birthday, weight, height. */
export function ProfilePersonalSheet() {
  const t = useTranslations('profileEdit');
  const { data: profile } = useUserProfile();

  if (!profile) {
    return (
      <div className="prof-edit-loading">
